...
function ProgressSteps({ currentStep }: { currentStep: number }) {
  const steps = [
    { id: 1, name: 'Upload Data', description: 'Import CSV files' },
    { id: 2, name: 'Configure Match', description: 'Set parameters' },
    { id: 3, name: 'Review Results', description: 'Analyze pairs' },
    { id: 4, name: 'Export', description: 'Download matches' }
  ];
        ...
        {steps.map((step, index) => (
          <div key={step.id} className="flex items-center">
            ...
            {index < steps.length - 1 && (
              <div className="w-12 h-0.5 bg-gray-300 mx-4 -mt-6"></div>
            )}
          </div>
        ))}
      </div>
...
function HeroSection({ mentorCount, menteeCount }: { mentorCount: number; menteeCount: number }) {
  ...
  return (
    <div className="text-center py-12 bg-blue-50 rounded-xl mb-8 border border-blue-100">
          ...
          <div className="bg-white rounded-lg p-6 shadow-sm border inline-block">
            <div className="flex items-center space-x-8">
              <div className="text-center">
                <div className="text-3xl font-bold text-blue-600">{mentorCount}</div>
                <div className="text-sm text-gray-600">Mentors Loaded</div>
              </div>
              <div className="text-center">
                <div className="text-3xl font-bold text-green-600">{menteeCount}</div>
                <div className="text-sm text-gray-600">Mentees Loaded</div>
              </div>
            </div>
          </div>
        ) : (
    ...
    <div className="bg-blue-50 border border-blue-200 rounded-lg mb-8">
      ...
      >
        <div>
          ...
          <p className="text-sm text-blue-700">Need help getting started? Download sample files and see format requirements</p>
        </div>
        <svg
      ...
      {isExpanded && (
        <div className="p-4 pt-0 border-t border-blue-200">
          <div className="grid md:grid-cols-2 gap-6">
            <div>
              <h4 className="font-medium text-blue-900 mb-2">📊 Sample Files</h4>
              <div className="space-y-2">
                ...
                >
                  <div className="font-medium text-sm">Download Sample Mentors CSV</div>
    ...
    <div className="max-w-6xl mx-auto">
      ...
      {mentors.length === 0 && mentees.length === 0 && (
        <div className="text-center py-12 bg-gray-50 rounded-xl border-2 border-dashed border-gray-300">
          ...
          <p className="text-gray-600 mb-4">Upload your mentor and mentee CSV files to begin the matching process</p>
